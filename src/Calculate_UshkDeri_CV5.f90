SUBROUTINE Calculate_UshkDeri_CV5_B(u_inf, v_inf, w_inf, p_inf, rho_inf, &
...
! Output variables: CV_1,CV_2,CV_3,CV_4,CV_5
  ...
  REAL(kind=8) :: u, v, w, rho, mach_ref, gamma
  REAL(kind=8) :: ub, vb, wb, pb, rhob
...
! Velocity normal to the shock after the shock
  ...
  pb = cv_5b/(gamma-1.d0)
  rhob = (u**2+v**2+w**2)*0.5d0*cv_5b
  tempb2 = rho*0.5d0*cv_5b
  ub = 2*u*tempb2
  vb = 2*v*tempb2
  wb = 2*w*tempb2
  tempb2 = rho_inf*(gamma+1.d0)*rhob/((gamma-1.d0)*masn2inf+2.d0)
  deltashkuinfb = shocknormalz*wb + shocknormaly*vb + shocknormalx*ub
  masn2infb = (1.0-(gamma-1.d0)*masn2inf/((gamma-1.d0)*masn2inf+2.d0))*&
&   tempb2 + gamma*2.d0*p_inf*pb/(gamma+1.d0) - delta_inf*2.d0*&
...
&   tempb2
  shocknormalyb = deltashkuinf*vb + v_inf*delta_infb + wallnormaly*&
&   tempb2
  shocknormalxb = deltashkuinf*ub + u_inf*delta_infb + wallnormalx*&
&   tempb2
...
&   wallnormalx*heta*xdxib
  cv_5b = 0.0_8
END SUBROUTINE Calculate_UshkDeri_CV5_B